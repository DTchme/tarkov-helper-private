...
public partial class MainWindow : Window
{
    ...
    private void OnProfileChanged(object? sender, ProfileType e)
    {
        ...
        try
        {
            ...
            else if (e == ProfileType.SeasonalPvp)
                RadioSeasonalPvp.IsChecked = true;
            else
    ...
    private async void Window_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        RadioSeasonalPvp.Checked += ProfileRadio_Checked;
    ...
    private async void ProfileRadio_Checked(object sender, RoutedEventArgs e)
    {
        ...
        var newProfile = sender == RadioPve
            ? ProfileType.Pve
            : sender == RadioSeasonalPvp
    ...
    private async void BtnResetProgress_Click(object sender, RoutedEventArgs e)
    {
        ...
        HideSettingsOverlay();
        await PerformQuestRebuildAsync(logPath, currentProfile);
    }
    ...
    private async Task PerformQuestRebuildAsync(string logPath, ProfileType profileType)
    {
        ...
        try
        {
            ...
            syncResult = await _logSyncService.SyncFromLogsAsync(
                logPath,
                progress,
                daysRange: 0,
                inferPrerequisites: false,
                rebuildMode: true);
        }
        catch (Exception ex)
        {
        ...
        if (syncResult.QuestsToComplete.Count == 0 && syncResult.InProgressQuests.Count == 0)
        {
            MessageBox.Show(
        ...
        var selectedChanges = SyncResultDialog.ShowResult(syncResult, this, out _);
        ...
        catch (Exception ex)
        {
            ...
            if (destructiveStepStarted && !string.IsNullOrWhiteSpace(backupPath))
            {
                try
                {
                    ...
                    await QuestProgressService.Instance.InitializeFromDbAsync(profileType);
                    await ObjectiveProgressService.Instance.LoadObjectiveProgressAsync();
                    restoreMessage = "\n\n변경 전 사용자 DB 백업으로 자동 복원했습니다.";
                }
        ...
        finally
        {
            if (wasMonitoring && Directory.Exists(logPath))
            {
                ...
                {
                    _log.Warning($"Failed to restart quest log monitoring after rebuild: {monitorEx.Message}");
                }
            }
        ...
        Dispatcher.Invoke(() =>
        {
            ...
            if (tasksByQuestId.TryGetValue(evt.QuestId, out var task))
            {
                ...
                QuestStatus? exactStatus = evt.EventType switch
                {
                    ...
                    QuestEventType.Started => QuestStatus.Active,
                    _ => null
                };
    ...
    private async void BtnUpdateApiData_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            TxtApiUpdateStatus.Foreground = GetStatusBrush("TextSecondaryBrush", Brushes.Gray);
            ...
            if (result.IsWarning)
            {
                ...
                TxtApiUpdateStatus.Foreground = GetStatusBrush("WarningBrush", Brushes.DarkOrange);
            }
            else if (result.Success)
            {
                ...
                {
                    TxtApiUpdateStatus.Text = result.Message;
                    TxtApiUpdateStatus.Foreground = GetStatusBrush("SuccessBrush", Brushes.Green);

                    // The database update event reloads DB readers asynchronously.
                    // Explicitly rebuild the progress lookup before a log sync can run,
                    // so it cannot use the previous quest list during that short window.
                    var currentProfile = ProfileService.Instance.CurrentProfile;

                    // Wait for the item/objective readers as well. The DatabaseUpdated event
                    // dispatches asynchronous reloads, so relying on the event alone can briefly
                    // leave the UI on the previous in-memory dataset.
                    await ItemDbService.Instance.RefreshAsync();
                    await QuestObjectiveDbService.Instance.RefreshAsync();

                    var progressService = QuestProgressService.Instance;
                    if (await progressService.InitializeFromDbAsync(currentProfile))
                    {
                        QuestGraphService.Instance.Initialize(progressService.AllTasks.ToList());
                    }
                    await ObjectiveProgressService.Instance.LoadObjectiveProgressAsync();
            ...
            else
            {
                ...
                TxtApiUpdateStatus.Foreground = GetStatusBrush("ErrorBrush", Brushes.IndianRed);
            }
        }
        catch (Exception ex)
        {
            _log.Error($"Manual API update failed: {ex}");
    ...
    /// <summary>
    /// Resolves a status brush without throwing when a theme resource is missing.
    /// A missing cosmetic resource must never terminate the application or hide the real API error.
    /// </summary>
    private Brush GetStatusBrush(string resourceKey, Brush fallback)
    {
