...
namespace TarkovHelper.Services
{
    ...
    public class LogSyncService : IDisposable
    {
        ...
        private static LogProfileKind DetectLogProfile(string content)
        {
            if (content.Contains("wsn-pvp-season-", StringComparison.OrdinalIgnoreCase))
                return LogProfileKind.SeasonalPvp;
            if (content.Contains("wsn-pve-", StringComparison.OrdinalIgnoreCase))
                return LogProfileKind.Pve;
            if (content.Contains("wsn-pvp-", StringComparison.OrdinalIgnoreCase))
                return LogProfileKind.Pvp;
            return LogProfileKind.Unknown;
        }

        private static bool MatchesProfile(LogProfileKind sourceProfile, ProfileType targetProfile)
        {
            return targetProfile switch
            {
                ProfileType.Pve => sourceProfile == LogProfileKind.Pve,
                ProfileType.Pvp => sourceProfile == LogProfileKind.Pvp,
                ProfileType.SeasonalPvp => sourceProfile == LogProfileKind.SeasonalPvp,
                _ => false
            };
        }

        private static async Task<LogProfileKind> DetectLogProfileFromFileAsync(string filePath)
        {
            try
            {
        ...
        /// </summary>
        private QuestLogEvent? ParseJsonBlock(string jsonString, string? sourceFile, LogProfileKind sourceProfile)
        {
            try
            {
                ...
                var templateTokens = templateId.Split(' ', StringSplitOptions.RemoveEmptyEntries);

                // Dynamically generated operational tasks use a shared template ID and append
                // the trader ID plus an instance index. Tarkov Helper's static quest database
                // cannot represent those instances, so do not report them as missing static quests.
                if (templateTokens.Length >= 4 &&
                    templateTokens[2].Length == 24 &&
                    int.TryParse(templateTokens[3], out _))
                {
                    return null;
                }

                // Extract quest ID (first token in templateId)
                var questId = templateTokens[0];
        ...
        public async Task<SyncResult> SyncFromLogsAsync(
            string logFolderPath,
            IProgress<string>? progress = null,
            int daysRange = 0,
            bool inferPrerequisites = false,
            bool rebuildMode = false)
        {
            ...
            if (result.SkippedOtherProfileEvents > 0)
            {
                progress?.Report($"다른 프로필/시즌 로그 {result.SkippedOtherProfileEvents}개를 제외했습니다");
            }
            ...
            if (events.Count == 0)
            {
                result.Errors.Add($"현재 {currentProfile} 프로필과 일치하는 퀘스트 이벤트를 찾을 수 없습니다");
                return result;
            }
            ...
            foreach (var kvp in questFinalStates)
            {
                ...
                switch (eventType)
                {
                    ...
                    case QuestEventType.Completed:
                        ...
                        if (rebuildMode || currentStatus != QuestStatus.Done)
                        {
                    ...
                    case QuestEventType.Failed:
                        // Only add if status will actually change
                        if (rebuildMode || currentStatus != QuestStatus.Failed)
                        {
                            questsToComplete.Add(new QuestChangeInfo
                            {
                                QuestName = task.Name,
                ...
                if (!inferPrerequisites || eventType == QuestEventType.Started)
                    continue;
            ...
            result.AlternativeQuestGroups = inferPrerequisites
                ? CollectAlternativeQuestGroups(progressService, questFinalStates, terminalStateQuests)
                : new List<AlternativeQuestGroup>();
            ...
            foreach (var kvp in questFinalStates)
            {
                ...
                {
                    // During a rebuild, the log is the source of truth even if the old
                    // database says this quest was already completed.
                    var currentStatus = progressService.GetStatus(task);
                    if (rebuildMode || currentStatus != QuestStatus.Done)
                    {
        ...
        public async Task ApplyQuestChangesAsync(List<QuestChangeInfo> changes)
        {
            ...
            if (batchChanges.Count > 0)
            {
                ...
                _log.Info($"Batch applied {batchChanges.Count} quest changes to {profileType}");
            }
