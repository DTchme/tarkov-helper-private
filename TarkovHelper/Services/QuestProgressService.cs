...
namespace TarkovHelper.Services
{
    ...
    public class QuestProgressService
    {
        ...
        public async Task<bool> InitializeFromDbAsync(ProfileType? profileType = null)
        {
            ...
            var actualProfile = profileType ?? ProfileService.Instance.CurrentProfile;
            var tasks = dbService.AllQuests
                .Where(t => QuestProfileScopeService.IsVisibleForProfile(t, actualProfile))
                .ToList();
            await InitializeAsync(tasks, actualProfile);
            IsLoadedFromDb = true;

            // V2 형식으로 진행 데이터 reconcile
            ReconcileProgressWithDb();

            System.Diagnostics.Debug.WriteLine($"[QuestProgressService] Initialized from DB with {tasks.Count} quests for {actualProfile}");
            return true;
        }
            ...
            ProfileType? profileType = null)
        {
                ...
                switch (status)
                {
                    case QuestStatus.Done:
                        ...
                        if (CompleteQuestBatchInternal(task, visited, changedItems))
                        {
                            ...
                            if (applyAlternativeConsequences && task.AlternativeQuests != null)
                            {
                                foreach (var altQuestName in task.AlternativeQuests)
                                {
                                    var altTask = GetTask(altQuestName) ?? GetTaskById(altQuestName);
            ...
            if (changedItems.Count > 0)
            {
                ...
                await _userDataDb.SaveQuestProgressBatchAsync(changedItems, profileType ?? _loadedProfile);
        ...
        public async Task ResetAllProgressAsync(ProfileType? profileType = null)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"[QuestProgressService] Quest progress cleared for {actualProfile}");
            ProgressChanged?.Invoke(this, EventArgs.Empty);
        }

        [Obsolete("Use ResetAllProgressAsync so log import cannot race with the database clear.")]
        public void ResetAllProgress()
        {
            _ = ResetAllProgressAsync();
        }
