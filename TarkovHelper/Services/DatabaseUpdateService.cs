...
public sealed class DatabaseUpdateService : IDisposable
{
    ...
    {
        if (_isUpdating)
            return new UpdateCheckResult(false, false, "이미 퀘스트 데이터 갱신이 진행 중입니다");
        ...
        {
            _log.Error($"Quest database refresh failed: {ex}");
...
public class UpdateCheckResult
{
    ...
    public UpdateCheckResult(bool success, bool wasUpdated, string message, bool isWarning = false)
    {
        Success = success;
