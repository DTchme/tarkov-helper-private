...
### 데이터 업데이트
퀘스트 데이터는 설정의 **현재 프로필 퀘스트 DB 갱신** 버튼으로 수동 갱신합니다.
현재 선택한 프로필에 따라 PvE 또는 일반 PvP 퀘스트 데이터가 저장됩니다. 프로필을 바꿔 사용할 때는 다시 갱신해야 합니다.
...
## 1.5.10 v6 - 기본 지도 확대 배율 조정

- 지도 기본 확대 배율을 100%에서 70%로 낮춤
- 초기 진입, 맵 변경, 보기 초기화, 저장값이 없는 맵의 기본 배율을 모두 70%로 통일
- 70%를 줌 프리셋에 추가
- 기존에 저장된 맵별 사용자 확대 배율은 그대로 유지
- 마커 좌표/크기 보정과 수동 확대/축소 범위는 변경하지 않음

## 1.5.10 v5 - Wiki 우선 퀘스트 동기화
현재 퀘스트 존재/기본 정보는 wikiwiki.jp/eft를 우선 기준으로 갱신하고, tarkov.dev는 구조화 메타데이터 보조 소스로 사용합니다.
